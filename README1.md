...
| 元数据库 | SQLite |
| 向量存储 | Chroma (Docker, 纯 HTTP 客户端，零额外依赖) |
...
- Python 3.10+
- Node.js 20+
- Docker Desktop（运行 Chroma 向量库，可选：也支持本地文件模式）
...
│   │   ├── ingestion/            # 文档摄入流水线
│   │   │   ├── parser.py         #   PDF/DOCX/Markdown 解析
│   │   │   ├── chunker.py        #   递归文本分块
...
| `CHUNK_OVERLAP` | 分块重叠（字符） | `50` |
| `SIMILARITY_THRESHOLD` | 冲突候选相似度阈值 | `0.85` |
